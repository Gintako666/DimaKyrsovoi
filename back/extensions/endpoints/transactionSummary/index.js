...
async function calculateTotal(type, startDate, endDate, transactionService) {
  const query = {
    filter: {
      value: type === "incoming" && { _lt: 0 } || type === "outgoing" && { _gt: 0 },
      _and: [
        {
          date: {
            _gt: startDate
...
async function calculateMonthlyData(startDate, endDate, transactionService) {
  const monthlyData = [];
